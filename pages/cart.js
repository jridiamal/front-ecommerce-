...
export default function CartPage() {
    ...
    if(cartProducts.length > 0){
      ...
      axios.post("/api/cart", { ids: [...new Set(ids)] })
        ...
        .catch(err => {
          ...
          setToast({message: "Erreur chargement produits", type: "error"});
        });
    } else {
      setProducts([]);
    }
  ...
  const groupedCart = cartProducts.reduce((acc, cartItem) => {
    ...
    if (!acc[key]) acc[key] = { _id: id, color, colorId, quantity: 0, cartItem: cartItem };
    acc[key].quantity += 1;
    return acc;
  }, {});
  ...
  async function goToPayment() {
    ...
    if(!name || name.length < 3) {
      setToast({message: "Nom invalide (minimum 3 caractères)", type: "error"});
      return;
    }
    
    if(!/^(2|4|5|9)\d{7}$/.test(phone)) {
      setToast({message: "Téléphone invalide. Format: 8 chiffres (2,4,5,9)", type: "error"});
      return;
    }
    
    if(!streetAddress || streetAddress.length < 5) {
      setToast({message: "Adresse invalide (minimum 5 caractères)", type: "error"});
      return;
    }
    ...
    try {
      ...
      console.log("Envoi checkout data:", {
      ...
      if (response.data.success) {
        ...
        // Rediriger vers la page de compte après 3 secondes
        setTimeout(() => {
          router.push("/account");
        }, 3000);
      ...
      setIsLoading(false);
      // Effacer le toast après 5 secondes
      setTimeout(() => {
        setToast(null);
      }, 5000);
    }
  ...
  if(status === "loading") {
    return (
      <>
        <Header />
        <Center>
          <div style={{padding:'100px', textAlign:'center', color: TEXT_MUTED}}>
            Chargement...
          </div>
        </Center>
      </>
        ...
        <ColumnsWrapper>
                    ...
                    {groupedItems.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx}>
                          <ProductInfoCell>
                            <ProductImageBox>
                              <img src={displayImage || "/default-product.jpg"} alt={product?.title}/>
                            </ProductImageBox>
                            <div>
                          ...
                          <td>
                            <MobileFlexRow>
                              <QuantityControls>
                                <QuantityButton 
                                  onClick={() => removeProduct(item.cartItem || item._id)}
                                  disabled={isLoading}
                                >
                                  -
                                </QuantityButton>
                                ...
                                <QuantityButton 
                                  onClick={() => addProduct(item.cartItem || item._id)}
                                  disabled={isLoading}
                                >
                                  +
                                </QuantityButton>
          ...
          {cartProducts.length > 0 && (
            <Box>
              ...
              />
              <StyledInput 
                placeholder="Téléphone (ex: 20000000)" 
                value={phone} 
